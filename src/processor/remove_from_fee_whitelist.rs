...
/// Removes a wallet from the fee-exempt list by closing its `AccessEntry` PDA.
/// Only closes entries currently holding `ACCESS_FEE_EXEMPT`, so it cannot be
/// used to drop a plain allow or deny entry.
///
/// Accounts:
